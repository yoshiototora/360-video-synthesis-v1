...
低コスト化を目的とした市販カメラの複数台合成手法も存在しますが、以下の課題がありました。：

- **被写体の不連続性**: 固定境界で機械的に接合するため、境界に位置する人物の身体が分断される
- **境界の振動**: 各フレームの画素値に依存して境界が不規則に変動し、VR酔いを引き起こす 。
- **筐体の映り込み**: 近接配置した隣接カメラの映り込みを完全に除去できない 。
